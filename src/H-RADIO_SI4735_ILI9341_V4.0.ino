...
// =====================PINS========================
#define ESP32_I2C_SDA    GPIO_NUM_4  // I2C bus pin on ESP32
#define ESP32_I2C_SCL    GPIO_NUM_0  // I2C bus pin on ESP32
#define RESET_PIN        GPIO_NUM_15
#define ENCODER_PIN_A    GPIO_NUM_32  // http://www.buxtronix.net/2011/10/rotary-encoders-done-properly.html
...
RetroBand bandRetro[] {
  ...
  "SW4",  "КВ4",  "MOSTLY DAY",   "В ОСНОВНОМ ДЕНЬ",  10,240,     0,160,   29,  13570,  18168,  13570,    1,  100,  5,    1,
  "SW5",  "КВ5",  "DAY",          "ДЕНЬ",             10,280,     0,200,   29,  18900,  26100,  18900,    1,  100,  5,    1,
  };
...
void Smeter() {
  ...
  while (met > 11 and tik < 9) {
    if (tik) tft.fillRect(Xsmtr + 20 + (tik * 12), Ysmtr + 38 , 10, 6, TFT_WHITE); else tft.fillRect(Xsmtr + 15, Ysmtr + 38 , 15, 6, TFT_WHITE);
    met -= 12;
    tik++;
  }
  while (met > 15 and tik < 15) {
    tft.fillRect(Xsmtr + 20 + ((tik - 9) * 16) + 108, Ysmtr + 38 , 14, 6, TFT_WHITE);
    met -= 16;
    tik++;
  }
  if (tik == 15 and met > 4) {
    tft.fillRect(Xsmtr + 20 + 204, Ysmtr + 38, 3, 6, TFT_WHITE);
  } else {
...
void VolumeIndicator(int val) {
  ...
  tft.fillRect(XVolInd + 15, YVolInd + 16 , (2 + val), 6, TFT_WHITE);
...
void DisplayRDS()  {
  ...
  if (( currentMode == FM) and ((FirstLayer) or (ThirdLayer))) {
    if ( currentFrequency != previousFrequency ) {
      previousFrequency = currentFrequency;
      tft.fillRect(XFreqDispl + 70, YFreqDispl + 57, 100, 16, TFT_BLACK);  // clear RDS text
    }
    if ((RDS) and  (NewSNR >= 12) and !VOLbut and !AGCgainbut) checkRDS(); else tft.fillRect(XFreqDispl + 70, YFreqDispl + 57, 100, 16, TFT_BLACK); // clear RDS text
  }
}
...
void DrawSmeter()  {
  ...
  String IStr;
  tft.setTextSize(1);
  tft.fillRect(Xsmtr + 2, Ysmtr + 6, 236, 46, TFT_GREY);
  tft.setTextColor(TFT_WHITE, TFT_GREY);
  tft.setTextDatum(BC_DATUM);
  for (int i = 0; i < 10; i++) {
...
void showRDSStation() {
  ...
  if ((FirstLayer) or (ThirdLayer)) {
    tft.setCursor(XFreqDispl + 73, YFreqDispl + 58);
...
void FreqDispl() {
  ...
  if (FirstLayer or ThirdLayer or SecondLayer) {
    ...
    if ((VOLbut) or (AGCgainbut)) {
      ...
      if (volDisp) tft.fillRect( XFreqDispl + d + 40, YFreqDispl + 20 , 40, y, TFT_BLACK); else tft.fillRect( XFreqDispl + d, YFreqDispl + 20 , 240, y, TFT_BLACK);
      tft.setTextColor(TFT_WHITE, TFT_BLACK);
      tft.setTextSize(3);
      if (VOLbut) {
...
void drawSCAN() {
  ...
  FreqDispl();
  drawList(L_SCAN, String(band[bandIdx].bandName) + " BAND SCANER");
